...
namespace Yumi
{    
    Shader::Shader(const String& name, const String& path, const Id& id)
    {
    }
    ...
    void Shader::Compile(const String& vertexSource, const String& fragmentSource)
    {
    ...
    void Shader::SetUniformMat4(const char* uniformName, const Matrix4& mat) const
    {
        const uint32_t id = glGetUniformLocation(m_ShaderId, uniformName);
        //glUniformMatrix4fv(id, 1, false, glm::value_ptr(mat));
    }

    void Shader::SetUniformVec4(const char* uniformName, const Vector4& vec) const
    {
        const uint32_t id = glGetUniformLocation(m_ShaderId, uniformName);
